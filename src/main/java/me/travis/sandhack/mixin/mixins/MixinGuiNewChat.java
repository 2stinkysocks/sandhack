...
import me.travis.sandhack.hack.hacks.chat.ClearChatbox;
    ...
    private int drawStringWithShadowMaybe(FontRenderer fontRenderer, String message, float x, float y, int color) {
        ...
        if (CustomChat.INSTANCE.customFont.getValue()) {
            if (CustomChat.INSTANCE.rainbow.getValue()) {
                Sandhack.GUI_FONT_MANAGER.drawStringRainbow(message, x, y, true);
            } else {
                Sandhack.GUI_FONT_MANAGER.drawStringWithShadow(message, x, y, color);
            }
        } else {
            if (CustomChat.INSTANCE.rainbow.getValue()) {
