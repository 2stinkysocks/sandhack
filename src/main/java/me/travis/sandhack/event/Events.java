...
import me.travis.sandhack.gui.alt.defult.GuiAltButton;
...
import me.travis.sandhack.util.elements.GLUProjection;
import me.travis.sandhack.util.elements.Timer;
...
public class Events implements Globals {
    ...
    public void onKeyPress(InputEvent.KeyInputEvent org.madmeg.wurstplus.event) {
        for(Hack hack : Sandhack.HACKS.getHacks()){
    ...
    public void onTick(TickEvent.ClientTickEvent event) {
        if (!nullCheck()) {
            Sandhack.HACKS.onTick();
        }
    }

    @CommitEvent(priority = me.travis.sandhack.event.processor.EventPriority.HIGH)
    public void onUpdateWalkingPlayer(UpdateWalkingPlayerEvent event) {
        if (nullCheck()) {
            return;
        }
        if (event.getStage() == 0) {
            Sandhack.ROTATION_MANAGER.updateRotations();
            Sandhack.POS_MANAGER.updatePosition();
        }
        if (event.getStage() == 1) {
            Sandhack.ROTATION_MANAGER.restoreRotations();
            Sandhack.POS_MANAGER.restorePosition();
        }
    }
    ...
    public void onRenderGameOverlayEvent(RenderGameOverlayEvent.Text event) {
        if (event.getType().equals(RenderGameOverlayEvent.ElementType.TEXT)) {
            ...
            Sandhack.HACKS.onRender2D(render2DEvent);
    ...
    public void onWorldRender(RenderWorldLastEvent event) {
        ...
        Sandhack.HACKS.onRender3D(render3dEvent);
        GlStateManager.glLineWidth(1.0f);
    ...
    public void onClientDisconnect(FMLNetworkEvent.ClientDisconnectionFromServerEvent event) {
        Sandhack.HACKS.onLogout();
        Sandhack.POP_MANAGER.onLogout();
    }

    @SubscribeEvent
    public void onClientConnect(FMLNetworkEvent.ClientConnectedToServerEvent event) {
        Sandhack.HACKS.onLogin();
    }

    @SubscribeEvent(priority = EventPriority.HIGHEST)
    public void onChatSent(ClientChatEvent event) {
        if (event.getMessage().startsWith(Sandhack.COMMANDS.getPrefix())) {
            event.setCanceled(true);
            try {
                mc.ingameGUI.getChatGUI().addToSentMessages(event.getMessage());
                if (event.getMessage().length() > 1) {
                    Sandhack.COMMANDS.executeCommand(event.getMessage().substring(1));
                } else {
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        Sandhack.SERVER_MANAGER.onPacketReceived();
        if (event.getPacket() instanceof SPacketEntityStatus) {
            SPacketEntityStatus packet = event.getPacket();
            try {
                if (packet.getOpCode() == 0x23 && packet.getEntity(mc.world) instanceof EntityPlayer) {
                    EntityPlayer player = (EntityPlayer) packet.getEntity(mc.world);
                    Sandhack.EVENT_PROCESSOR.addEventListener(new TotemPopEvent(player));
                    Sandhack.POP_MANAGER.onTotemPop(player);
                }
            ...
            packet.getEntries().stream().filter(Objects::nonNull).filter(data -> !Strings.isNullOrEmpty(data.getProfile().getName()) || data.getProfile().getId() != null).forEach(data -> {
                ...
                switch (packet.getAction()) {
                    case ADD_PLAYER: {
                        ...
                        Sandhack.EVENT_PROCESSOR.addEventListener(new ConnectionEvent(0, id, name));
                        break;
                    }
                    case REMOVE_PLAYER: {
                        ...
                        if (entity != null) {
                            ...
                            Sandhack.EVENT_PROCESSOR.addEventListener(new ConnectionEvent(1, entity, id, logoutName));
                            break;
                        }
                        Sandhack.EVENT_PROCESSOR.addEventListener(new ConnectionEvent(2, id, null));
                    }
                    default:
                        break;
                }
            ...
            Sandhack.SERVER_MANAGER.update();
        }
    }
