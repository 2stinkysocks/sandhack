package me.travis.sandhack.gui.chat;

import com.google.common.collect.Lists;
import me.travis.sandhack.Sandhack;
import me.travis.sandhack.hack.hacks.chat.CustomChat;
import me.travis.sandhack.hack.hacks.player.PlayerSpoofer;
import me.travis.sandhack.util.AnimationUtil;
...
public class GuiChat extends GuiNewChat implements Globals {
    private static final Logger LOGGER = Sandhack.LOGGER;
                                ...
                                if (CustomChat.INSTANCE.smoothChat.getValue() && i1 <= newLines) {
                                    ...
                                    if (CustomChat.INSTANCE.rainbow.getValue()){
                                        if (CustomChat.INSTANCE.customFont.getValue()) {
                                            Sandhack.GUI_FONT_MANAGER.drawStringRainbow(s, 0.0f + (CustomChat.INSTANCE.type.is("Vertical") ? messageAdd : 0), (j2 - 8), true);
                                        } else {
                                        ...
                                        if (CustomChat.INSTANCE.customFont.getValue()) {
                                            Sandhack.GUI_FONT_MANAGER.drawStringWithShadow(s, 0.0f + (CustomChat.INSTANCE.type.is("Vertical") ? messageAdd : 0), (j2 - 8), 16777215 + ((l1) << 24));
                                        } else {
                                    ...
                                    if (CustomChat.INSTANCE.rainbow.getValue()){
                                        if (CustomChat.INSTANCE.customFont.getValue()) {
                                            Sandhack.GUI_FONT_MANAGER.drawStringRainbow(s, 0.0f, (j2 - 8), true);
                                        } else {
                                            this.mc.fontRenderer.drawStringWithShadow(s, 0.0f, (j2 - 8), Rainbow.getColour().getRGB());
                                        }
                                    } else {
                                        if (CustomChat.INSTANCE.customFont.getValue()) {
                                            Sandhack.GUI_FONT_MANAGER.drawStringWithShadow(s, i2, (j2 - 8), 16777215 + (l1 << 24));
                                        } else {
