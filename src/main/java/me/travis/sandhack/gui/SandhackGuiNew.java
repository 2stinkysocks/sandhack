...
public class SandhackGuiNew extends GuiScreen {
    ...
    public SandhackGuiNew() {
        categoryComponents = new ArrayList<>();
        int startX = 10;
        for (Hack.Category category : Sandhack.HACKS.getCategories()) {
    ...
    public void onGuiClosed() {
        ...
        Sandhack.CONFIG_MANAGER.saveConfig();
    }
    ...
    private void animate(ScaledResolution sr){
        final int deltaTime = Sandhack.RENDER_UTIL_2D.getDeltaTime();
        for(CategoryComponent c : categoryComponents) {
