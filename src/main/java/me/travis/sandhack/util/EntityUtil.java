...
public class EntityUtil implements Globals {
    ...
    public static boolean isntValid(final Entity entity, final double range) {
        return entity == null || !entity.isEntityAlive() || entity.equals(mc.player) || (entity instanceof EntityPlayer && Sandhack.FRIEND_MANAGER.isFriend(entity.getName())) || mc.player.getDistanceSq(entity) > (range * range);
    }
    ...
    public static Color getColor(final Entity entity, final int red, final int green, final int blue, final int alpha, final boolean colorFriends) {
        ...
        if (entity instanceof EntityPlayer) {
            if (colorFriends && Sandhack.FRIEND_MANAGER.isFriend(entity.getName())) {
                color = new Color(0.33f, 1.0f, 1.0f, alpha / 255.0f);
            }
            if (colorFriends && Sandhack.ENEMY_MANAGER.isEnemy(entity.getName())) {
