package me.travis.sandhack.hack.hacks.client;

import com.mojang.realmsclient.gui.ChatFormatting;
import me.travis.sandhack.Sandhack;
import me.travis.sandhack.event.events.Render2DEvent;
...
import me.travis.sandhack.util.HudUtil;
...
public class Hud extends Hack {
    ...
    private void runEvents(){
        ...
        while (true){
            if(start > System.currentTimeMillis() - 100){
                ...
                Sandhack.EVENT_PROCESSOR.postEvent(testEvent);
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        if (this.lagNot.getValue() && Sandhack.SERVER_MANAGER.isServerNotResponding()) {
            this.drawStringCenterX(ChatFormatting.RED + "Server is not responding " + Math.round(Sandhack.SERVER_MANAGER.serverRespondingTime() / 1000.0f), 56);
        }
    ...
    private void doBottomRight() {
        if (this.arrayList.getValue()) {
            List<Hack> hacks = Sandhack.HACKS.getSortedHacks(false, this.customFont.getValue());
            int y = scaledResolution.getScaledHeight() - (11 * hacks.size()) + 2 + (11 * Sandhack.HACKS.getDrawnHacks().size());
            for (Hack hack : hacks) {
                if (Sandhack.HACKS.isDrawHack(hack)) continue;
    ...
    private void doHelper() {
        ...
        int y = 200;
        String ca = (Sandhack.HACKS.ishackEnabled("Crystal Aura") ? ChatFormatting.GREEN + "1" : ChatFormatting.RED + "0") + ChatFormatting.RESET;
        String holefill = (Sandhack.HACKS.ishackEnabled("Hole Fill") ? ChatFormatting.GREEN + "1" : ChatFormatting.RED + "0") + ChatFormatting.RESET;
        String trap = (Sandhack.HACKS.ishackEnabled("Trap") ? ChatFormatting.GREEN + "1" : ChatFormatting.RED + "0") + ChatFormatting.RESET;
        String surround = (Sandhack.HACKS.ishackEnabled("Surround") ? ChatFormatting.GREEN + "1" : ChatFormatting.RED + "0") + ChatFormatting.RESET;
        String ka = (Sandhack.HACKS.ishackEnabled("Kill Aura") ? ChatFormatting.GREEN + "1" : ChatFormatting.RED + "0") + ChatFormatting.RESET;
    ...
    private void renderFriends() {
        ...
        for (EntityPlayer player : mc.world.playerEntities) {
            if (Sandhack.FRIEND_MANAGER.isFriend(player.getName())) {
    ...
    private void doTopleft() {
        ...
        if (watermark.getValue()) {
            drawString(ChatFormatting.GOLD + Sandhack.MODNAME + ChatFormatting.RESET
                    + " v" + Sandhack.MODVER, 10, y);
            y += 16;
        }
        if (fps.getValue()) {
    ...
    */
    ...
    private int getRightX(String string, int x) {
        if (this.customFont.getValue()) {
            return scaledResolution.getScaledWidth() - x - Sandhack.GUI_FONT_MANAGER.getTextWidth(string);
        } else {
    ...
    private void drawString(String string, int x, int y) {
        if (this.customFont.getValue()) {
            Sandhack.GUI_FONT_MANAGER.drawStringWithShadow(string, x, y, fontColour.getValue().getRGB());
        } else {
    ...
    private void drawString(String string, int x, int y, int colour) {
        if (this.customFont.getValue()) {
            Sandhack.GUI_FONT_MANAGER.drawStringWithShadow(string, x, y, colour);
        } else {
    ...
    private int getStringWidth(String string) {
        if (this.customFont.getValue()) {
            return Sandhack.GUI_FONT_MANAGER.getTextWidth(string);
        } else {
    ...
    private void drawStringCenterX(String string, int y) {
        if (this.customFont.getValue()) {
            int x = (scaledResolution.getScaledWidth() / 2) - (Sandhack.GUI_FONT_MANAGER.getTextWidth(string) / 2);
            Sandhack.GUI_FONT_MANAGER.drawStringWithShadow(string, x, y, fontColour.getValue().getRGB());
        } else {
