...
import me.travis.sandhack.util.elements.NametagRenderer;
...
public class Nametags extends Hack {
    ...
    public void renderNametag(EntityPlayer player, double x, double y, double z) {
        ...
        NetworkPlayerInfo npi = mc.player.connection.getPlayerInfo(player.getGameProfile().getId());
        boolean isFriend = Sandhack.FRIEND_MANAGER.isFriend(player.getName());
        boolean isEnemy = Sandhack.ENEMY_MANAGER.isEnemy(player.getName());
        String name = ((isFriend || isEnemy) && player.isSneaking() ? SECTIONSIGN + "9" : SECTIONSIGN + "r")
                ...
                + (popCounter.getValue() ? " " + SECTIONSIGN + this.getPop(Sandhack.POP_MANAGER.getTotemPops(player))
                + SECTIONSIGN + "r" : "");
        name = name.replace(".0", "");
        ...
        if (customFont.getValue()) {
            width = Sandhack.GUI_FONT_MANAGER.getTextWidth(name) / 2 + 1;
        } else {
            width = var13.getStringWidth(name) / 2;
        }
        int color = Color.BLACK.getRGB();
        int outlineColor = Sandhack.FRIEND_MANAGER.isFriend(player.getName()) ? outlineColourFriend.getValue().getRGB()
                : Sandhack.ENEMY_MANAGER.isEnemy(player.getName()) ? outlineColourEnemy.getValue().getRGB()
        ...
        if (!far) {
            RenderUtil.drawTriangleOutline(width - Sandhack.GUI_FONT_MANAGER.getTextWidth(name) / 2f, arrowPos.getValue(), 5, 2, 1, outlineWidth.getValue().floatValue(), outlineColor);
        }
        if (customFont.getValue()) {
            Sandhack.GUI_FONT_MANAGER.drawStringWithShadow(name, -width, 13 + textOffset.getValue(), fontColour.getValue().getRGB());
        } else {
    ...
    private void renderDurabilityText(ItemStack stack, int x) {
        if (stack.getItem() instanceof ItemArmor || stack.getItem() instanceof ItemSword
            ...
            if (customFont.getValue()) {
                Sandhack.GUI_FONT_MANAGER.drawStringWithShadow(dmg + "%", x * 2 + 4, -7 - 10,
    ...
    public void renderItem(EntityPlayer player, ItemStack stack, int x, int y, int nameX, boolean showHeldItemText) {
        ...
        if (!shownItem && showHeldItemText) {
            if (customFont.getValue()) {
                Sandhack.GUI_FONT_MANAGER.drawStringWithShadow(stack.getDisplayName().equalsIgnoreCase("Air") ? "" : stack.getDisplayName(), nameX * 2 + 95 - (Sandhack.GUI_FONT_MANAGER.getTextWidth(stack.getDisplayName()) / 2f), y - 37, -1);
            } else {
    ...
    public void renderEnchantText(EntityPlayer player, ItemStack stack, int x, int y) {
        ...
        if (stack.getItem() instanceof ItemArmor || stack.getItem() instanceof ItemSword
            ...
            if (durability.getValue()) {
                ...
                int dmg = 100 - (int) (red * 100);
                if (customFont.getValue()) {
                    Sandhack.GUI_FONT_MANAGER.drawStringWithShadow(dmg + "%", x * 2 + 4, y - 10,
                            (new Color(red, green, 0)).getRGB());
                } else {
        ...
        if (simple.getValue()) {
            if (isMaxEnchants(stack)) {
                ...
                if (simple.getValue()) {
                    if (customFont.getValue()) {
                        Sandhack.GUI_FONT_MANAGER.drawStringWithShadow("Max", x * 2 + 7, yCount + 24, Colour.RED.getRGB());
                    } else {
        ...
        for (int index = 0; index < enchants.tagCount(); ++index) {
            ...
            if (enc != null) {
                ...
                GL11.glScalef(1f, 1f, 0);
                if (customFont.getValue()) {
                    Sandhack.GUI_FONT_MANAGER.drawStringWithShadow(encName, x * 2 + 3, yCount, -1);
                } else {
