...
import me.travis.sandhack.event.events.JesusEvent;
...
public class Jesus extends Hack {
    ...
    public void updateWalkingPlayer(UpdateWalkingPlayerEvent event) {
        if (nullCheck() || Sandhack.HACKS.ishackEnabled("Freecam")) {
            return;
        }
    ...
    public void sendPacket(PacketEvent.Send event) {
        if (event.getPacket() instanceof CPacketPlayer && Sandhack.HACKS.ishackEnabled("Freecam")
