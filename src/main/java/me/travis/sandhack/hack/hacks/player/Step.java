...
import me.travis.sandhack.event.events.StepEvent;
...
public class Step extends Hack {
    ...
    public void onStep(StepEvent event) {
        if (nullCheck()) return;
        if (mc.player.onGround && !mc.player.isInsideOfMaterial(Material.WATER) && !mc.player.isInsideOfMaterial(Material.LAVA) && mc.player.collidedVertically && mc.player.fallDistance == 0.0f && !mc.gameSettings.keyBindJump.pressed && !mc.player.isOnLadder() && !Sandhack.HACKS.ishackEnabled("Speed")) {
