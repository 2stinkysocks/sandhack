...
import me.travis.sandhack.event.events.ConnectionEvent;
...
public class LogSpots extends Hack {
        ...
        if (event.getStage() == 0) {
            ...
            if (name != null && uuid != null) {
                this.spots.add(new LogoutPos(name, uuid, entity, Math.round(entity.getHealth() + entity.getAbsorptionAmount()),
                        entity.getHeldItemOffhand().getItem() == Items.TOTEM_OF_UNDYING, Sandhack.POP_MANAGER.getTotemPops(entity)));
            }
        }
    }
    ...
    private void renderNameTag(String name, double x, double yi, double z, float delta, double xPos, double yPos, double zPos, double hp, boolean hasTotem, int pops) {
        ...
        int width = Sandhack.GUI_FONT_MANAGER.getTextWidth(displayTag) / 2;
        int width2 = Sandhack.GUI_FONT_MANAGER.getTextWidth(displayTag2) / 2;
        ...
        GlStateManager.enableBlend();
        RenderUtil.drawRect(-width2 - 2, -(Sandhack.GUI_FONT_MANAGER.getTextHeight() + 1), (float) width2 + 2.0f, 14f, 0x55000000);
        GlStateManager.disableBlend();
        Sandhack.GUI_FONT_MANAGER.drawStringWithShadow(displayTag, -width, -(Sandhack.GUI_FONT_MANAGER.getTextHeight() - 1), colour.getValue().getRGB());
        Sandhack.GUI_FONT_MANAGER.drawStringWithShadow(displayTag2, -width2, -(Sandhack.GUI_FONT_MANAGER.getTextHeight() - 1 - 12), colour.getValue().getRGB());
