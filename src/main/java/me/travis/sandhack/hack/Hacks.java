package me.travis.sandhack.hack;
...
import me.travis.sandhack.util.ReflectionUtil;
...
public final class Hacks implements Globals {
    ...
    public Hacks() {
        ...
        try {
            ArrayList<Class<?>> modClasses = ReflectionUtil.getClassesForPackage("me.travis.sandhack.hack.hacks");
    ...
    public final void onUnload() {
        ...
        this.hacks.forEach(Sandhack.EVENT_PROCESSOR::removeEventListener);
    ...
    public final void onKeyDown(int key) {
        if (key <= 0 || mc.currentScreen instanceof SandhackGuiNew) {
            return;
        }
        for (Hack hack : this.hacks) {
    ...
    public final List<Hack> getSortedHacks(boolean reverse, boolean customFont) {
        if (customFont) {
            return this.getEnabledHacks().stream().sorted(Comparator.comparing(hack ->
                    Sandhack.GUI_FONT_MANAGER.getTextWidth(hack.getFullArrayString())
